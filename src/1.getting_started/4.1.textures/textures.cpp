...
// vertex to create some interesting images. However, to get a fair bit of
// realism we'd have to have many vertices so we could specify a lot of colors.
// This takes up a considerable amount of extra overhead, since each model needs
// a lot more vertices and for each vertex a color attribute as well. What
// artists and programmers generally prefer is to use a texture. A texture is a
// 2D image (even 1D and 3D textures exist) used to add detail to an object;
// think of a texture as a piece of paper with a nice brick image (for example)
// on it neatly folded over your 3D house so it looks like your house has a
// stone exterior. Because we can insert a lot of detail in a single image, we
...
void generate_colored_triangle_with_texture(float **vertices, int *nverts,
                                            int **indices, int *n_ind);
...
// render loop
void render_loop(GLFWwindow *window, Shader &ourShader, unsigned int VAO,
